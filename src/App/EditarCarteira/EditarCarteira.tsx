...
const EditarCarteira: React.FC = () => {
  ...
  const { carteiras, atualizarCarteira, removerCarteira } = useCarteira();
  
  const [carteira, setCarteira] = useState(carteiras.find(c => c.id === id));
  const [nomeCarteira, setNomeCarteira] = useState(carteira?.nome || "");
  const [valorCarteira, setValorCarteira] = useState(carteira?.valor || 0);
  const [iconeEscolhido, setIconeEscolhido] = useState(carteira?.icone || "wallet");
  ...
  useEffect(() => {
    const carteiraAtual = carteiras.find(c => c.id === id);
    if (carteiraAtual) {
      setCarteira(carteiraAtual);
      setNomeCarteira(carteiraAtual.nome);
      setValorCarteira(carteiraAtual.valor);
      setIconeEscolhido(carteiraAtual.icone);
    }
  }, [carteiras, id]);
  ...
  const handleDepositar = () => {
    const valorDeposito = parseFloat(inputValor.replace(',', '.'));
    if (!isNaN(valorDeposito) && valorDeposito > 0 && valorDeposito <= saldoDisponivel) {
      const novoValor = valorCarteira + valorDeposito;
      setValorCarteira(novoValor);
      // Atualizar imediatamente no contexto
      if (id) {
  ...
  const handleSaque = () => {
    const valorSaque = parseFloat(inputSaque.replace(',', '.'));
    if (!isNaN(valorSaque) && valorSaque > 0 && valorSaque <= valorCarteira) {
      const novoValor = valorCarteira - valorSaque;
      setValorCarteira(novoValor);
      // Atualizar imediatamente no contexto
      if (id) {
        atualizarCarteira(id, {
  ...
  const handleExcluirCarteira = () => {
    if (id && window.confirm('Tem certeza que deseja excluir esta carteira?')) {
      removerCarteira(id);
      navigate('/carteira');
    }
  };
      ...
      <>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <button onClick={() => navigate('/carteira')}>Voltar</button>
          </div>
        </div>
      </>
    );
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: '14px', color: '#64748b' }}>Saldo da carteira: </span>
                <span style={{ fontSize: '16px', fontWeight: '600', color: '#0065F5' }}>
                  R$ {valorCarteira.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
              ...
              <input
                ...
                placeholder="0,00"
                value={inputSaque}
                onChange={(e) => setInputSaque(e.target.value)}
                style={{
                  width: '100%',
                  padding: '12px',
                  border: '1px solid #e2e8f0',
